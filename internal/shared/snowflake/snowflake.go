...
// ErrOverflow is returned when parsing would result in integer overflow
var ErrOverflow = errors.New("value would overflow int64")
...
// IDs
func NewNode(node int64) (*NodeImpl, error) {
	...
	n.epoch = time.Unix(Epoch/1000, (Epoch%1000)*1000000)
...
func (id *ID) Scan(value any) error {
	...
	case uint64:
		if v > math.MaxInt64 {
			return fmt.Errorf("value %d exceeds maximum ID", v)
		}
	...
	case []byte:
		...
		if err != nil {
			return fmt.Errorf("failed to parse []byte as ID: %w", err)
		}
		*id = ID(i)
		return nil
	case string:
		...
		if err != nil {
			return fmt.Errorf("failed to parse string as ID: %w", err)
		}
		*id = ID(i)
		return nil
	default:
...
func IfNull(id ID, defaultID ID) ID {
	if id == 0 {
		return defaultID
	}
