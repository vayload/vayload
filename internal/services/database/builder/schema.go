...
import (
	...
	"github.com/vayload/vayload/internal/services/database/grammar"
...
func (s *SchemaBuilder) executeExists(query string) (bool, error) {
	// Aquí necesitarías acceso al *sqlx.DB para hacer Query
	// Por ahora retornamos un placeholder
	// En tu implementación real, necesitarías:
	// row := s.connection.DB().QueryRow(context.Background(), query)
	// var exists int
	// err := row.Scan(&exists)
	// return exists > 0, err
