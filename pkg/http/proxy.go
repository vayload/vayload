...
var ignoredHeaders = map[string]bool{
	"Host":            true,
	"Accept-Encoding": true, // Forzar texto plano
	"Content-Length":  true,
	"Connection":      true,
	"Keep-Alive":      true,
	"Te":              true,
...
type ProxyRequestHook func(req vayload.HttpRequest, proxyReq *http.Request) error
type ProxyResponseHook func(req vayload.HttpRequest, proxyRes *http.Response) error
type BodyModifierHook func(req vayload.HttpRequest, body *map[string]any) error
...
func (proxy *HttpProxy) HandleWithBodyHook(
	req vayload.HttpRequest,
	res vayload.HttpResponse,
	path string,
...
func copyHeaders(res vayload.HttpResponse, proxyRes *http.Response) {
	for k, v := range proxyRes.Header {
		if len(v) > 0 {
			res.SetHeader(k, v[0])
		}
