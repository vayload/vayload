...
import (
	...
	"github.com/vayload/vayload/pkg/utils"
)

// LogLevel defines the minimum severity level for error logging
type LogLevel int
...
func ErrWrapping(cause error, details ...any) *Err {
	...
	if errors.As(cause, &httErr) {
		// Preservar detalles originales si no se proporcionan nuevos
...
func HttpErrorHandler(req vayload.HttpRequest, res vayload.HttpResponse, err error) error {
